...
public class MyDBClient extends Client {
    ...
    protected void handleResponse(byte[] bytes, NIOHeader header) {
        ...
        try {
            JSONObject response = new JSONObject(new String(bytes, SingleServer
                    .DEFAULT_ENCODING));
            this.outstanding.put(response.getLong(MyDBClient.Keys.REQNUM.toString()),
                    response);
            synchronized (this.outstanding) {
                this.outstanding.notify();
            }
            Callback callback = callbacks.get(response.getLong(MyDBClient.Keys
                    .REQNUM.toString()));
            if(callback!=null) {
    ...
    public void callbackSend(InetSocketAddress isa, String request,
        ...
        try {
            JSONObject json = new JSONObject().put(MyDBClient.Keys.REQNUM.toString(),
                    enqueue(request)).put(MyDBClient.Keys.REQUEST.toString(), request);
            this.callbacks.put(json.getLong(MyDBClient.Keys.REQNUM.toString()), callback);
    ...
    protected String waitForResponse(Object preparedRequest) {
        long id = 0;
        try {
            id = ((JSONObject)preparedRequest).getInt(MyDBClient.Keys.REQNUM.toString
                    ());
        } catch (JSONException e) {
            e.printStackTrace();
        }
        synchronized (outstanding) {
            ...
            try {
                return outstanding.remove(id).getString(MyDBClient.Keys.RESPONSE.toString());
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
